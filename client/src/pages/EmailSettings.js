...
const EmailSettings = () => {
  ...
  const testConnection = async () => {
    setTesting(true);
    try {
      const response = await api.post('/api/email/test-connection', emailConfig);
  ...
  const processEmail = async () => {
    setTesting(true);
    try {
      const response = await api.post('/api/email/process-here-is-receipt-fixed');
      if (response.data.success) {
